  ...
  constructor(mod, opts) {
    ...
    if (mod[handler]) {
      ...
      if (!this.lambdaModule.lambdaFunction) {
        throw new Error(
          "Invalid input. Expected either lambda module (with exposed handler) " +
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        if (this.handler) {
          ...
          const lambda = this.lambdaModule.region
            ? new Lambda.LambdaClient({ region: this.lambdaModule.region })
            : new Lambda.LambdaClient();
          ...
          lambda
            ...
            .then(function (data) {
              if (data.FunctionError) {
                return callback(
                  Object.assign(
                    new Error(JSON.parse(data.Payload).errorMessage),
                    data
                  )
                );
              }
